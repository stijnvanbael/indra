...
class Daemon {
  ...
  late WorkerPool _workerPool;
  late ScriptRepository _scriptRepository;
  String host = '0.0.0.0';
  int port = 8080;
  int numberOfWorkers = 1;
  String workingDir;
  late ClosedAsyncPatternMatcher<Request, Response> requestMatcher;

  Daemon({this.workingDir = '~/indra/scripts'}) {
    requestMatcher = asyncMatcher<Request, Response>()
        .when2(
          _matchRequest('GET', '/jobs/:name/:number/output'),
          (Map<String, String> headers, body) =>
              _output(headers['name']!, headers['number']!),
        )
        .when2(
        ...
        .when2(
          ...
          (Map<String, String> headers, String body) =>
              _schedule(headers['name']!, _fromJson(body)),
        )
        .when2(
          _matchRequest('DELETE', '/jobs/:name/:number'),
          (Map<String, String> headers, body) =>
              _cancel(headers['name']!, headers['number']!),
        )
  ...
  static TransformingPredicate<Request,
          Future<Pair<Map<String, String>, String>>>
      _matchRequest(String method, String pathExpression) {
    StringPattern pathPattern = StringPattern(pathExpression);
    return predicate(
      (Request request) =>
          request.method == method &&
          pathPattern.matches(request.requestedUri.path),
      (Request request) async => Pair(
          _extractHeaders(request, pathPattern), await request.readAsString()),
  ...
  static Map<String, String> _fromJson(String body) => _isNotEmpty(body)
      ? json.decode(body) as Map<String, String>
      : <String, String>{};
