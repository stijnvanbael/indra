...
String toStringFormat(dynamic value) =>
...
class Prompt {
  ...
  static int _selectMenu<T>(
    ...
    var defaultChoice =
        defaultValue != null ? values.indexOf(defaultValue) + 1 : null;
  ...
  static int _promptChoice(List values, int? defaultChoice) {
    ...
    if (choice == null || choice <= 0 || choice > values.length) {
      output.showMessage(
          'Invalid choice "$line", valid values are 1-${values.length}, please try again: ');
  ...
  static bool _promptConfirm(bool? defaultChoice) {
    ...
    if (choice == null) {
      output.showMessage(
          'Invalid choice "$line", valid values are y or n, please try again: ');
